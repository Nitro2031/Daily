...
## メニュー（バックアップ・設定）
- ヘッダーの v-app-bar にメニュー（v-menu）を配置する。
- メニューには以下の項目を含める：
  - バックアップを保存（データを JSON としてエクスポート）
  - バックアップをリストア（JSON を読み込み、IndexedDB に反映）
  - 設定（アプリ設定画面を開く）
- バックアップ操作はユーザーが誤操作しないよう、確認ダイアログを表示する。
- モバイル操作を前提に、タップ領域は 40px 以上を確保する。
